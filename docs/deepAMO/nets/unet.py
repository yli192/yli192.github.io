...
def build_net(
    ...
    if channels_first:
        inputs = Input((input_channels,) + image_size)
    else:
        inputs = Input(image_size + (input_channels,))
...
class UnetFunctions(object):
    ...
    def local_conv(self, num_filters, dilation_rate, name=None):
    ...
    def conv_depth(self, num_filters, dilation_rate=None, name=None):
        ...
        def composite_layer(x):
            for c in range(self.convs_per_depth):
                if c != self.convs_per_depth - 1:
                    x = self.local_conv(num_filters=num_filters, dilation_rate=dilation_rate)(x)
                else:
                    x = self.local_conv(num_filters=num_filters, dilation_rate=dilation_rate, name=name)(x)
            return x

        return composite_layer

    def contract(self, depth):
        """Returns a contraction layer."""

        def composite_layer(x):
            name = 'contr_%d' % depth
            num_filters = self.base_num_filters * 2 ** depth
            x = self.conv_depth(num_filters=num_filters, name=name)(x)
